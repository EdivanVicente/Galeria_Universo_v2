...
function updateGallery() {
  ...
  const galleryCard = mainImage.closest(".gallery-card");
  const fileNameMatch = item.src.match(/\/([^/]+)\.\w+$/);
  const uniqueImageClass = fileNameMatch ? fileNameMatch[1] : '';
  ...
  if (isSvg) {
    ...
    mainImage.classList.remove('svg-file');
  }
  ...
  if (item.caption && item.caption.length > 190) {
    ...
    descriptionWrapper.classList.remove("extra-long-description", "short-description");
  } else {
    descriptionWrapper.classList.add("short-description");
    descriptionWrapper.classList.remove("long-description", "extra-long-description");
  }
